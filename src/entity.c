...
static void en_tag_entity(struct game * const g)
{
	...
	for (int i = 0; i != EN_ENT_MAX; ++i) {
		...
		if (EN_CAMERA_ID == i) {
			...
			++count;
		} else if (EN_BLOCK_TAU_ID == i) {
			ent->tag = EN_BLOCK_TAG;
			ent->id = EN_BLOCK_TAU_ID;
			++count;
		} else if (EN_BLOCK_OMICRON_ID == i) {
			ent->tag = EN_BLOCK_TAG;
			ent->id = EN_BLOCK_OMICRON_ID;
			++count;
...
static void en_init_aframes(struct game * const g)
{
	...
	en_init_block_aframes(g, EN_BLOCK_TAU_ID);
...
)
{
	if (
		...
		(EN_BLOCK_TAU_ID      != id_block) &&
		(EN_BLOCK_OMICRON_ID  != id_block) &&
	...
	struct entity * const tau_platform = &g->ents[EN_PLATFORM_TAU_ID];
	struct entity * const omicron_platform = &g->ents[EN_PLATFORM_OMICRON_ID];
	...
	if (EN_BLOCK_ETA_ID == id_block) {
	...
	} else if (EN_BLOCK_IOTA_ID == id_block) {
		iota_platform->blockno = EN_BLOCK_IOTA_ID;
		block->flags = 0;
		block->name = EN_BLOCK_IOTA_NM;
		block->platfno = EN_PLATFORM_IOTA_ID;
		block->blockno = EN_BLOCK_IOTA_ID;
		block->width = GAME_BLOCK_WIDTH;
		block->height = 0.25f * GAME_BLOCK_HEIGHT;
		block->xpos = iota_platform->xpos;
		block->ypos = (
				iota_platform->ypos -
		...
		block->ypos = (
				...
				(0.5f * block->height) -
				(40.0f * GAME_SONIC_HEIGHT)
		);
	} else if (EN_BLOCK_TAU_ID == id_block) {
		tau_platform->blockno = EN_BLOCK_TAU_ID;
		block->flags = 0;
		block->name = EN_BLOCK_TAU_NM;
		block->platfno = EN_PLATFORM_TAU_ID;
		block->blockno = EN_BLOCK_TAU_ID;
		block->width = GAME_BLOCK_WIDTH;
		block->height = 0.25f * GAME_BLOCK_HEIGHT;
		block->xpos = tau_platform->xpos;
		block->ypos = (
				tau_platform->ypos -
				(0.5f * tau_platform->height) -
				(0.5f * block->height) -
				(3.0f * GAME_SONIC_HEIGHT)
		);
	} else if (EN_BLOCK_OMICRON_ID == id_block) {
		omicron_platform->blockno = EN_BLOCK_OMICRON_ID;
		block->flags = 0;
		block->name = EN_BLOCK_OMICRON_NM;
		block->platfno = EN_PLATFORM_OMICRON_ID;
		block->blockno = EN_BLOCK_OMICRON_ID;
		block->width = GAME_BLOCK_WIDTH;
		block->height = 0.25f * GAME_BLOCK_HEIGHT;
		block->xpos = omicron_platform->xpos;
		block->ypos = (
				omicron_platform->ypos -
				(0.5f * omicron_platform->height) -
				(0.5f * block->height) -
				(2.0f * GAME_SONIC_HEIGHT)
		);
	}
...
static void en_update_camera(struct game * const g)
{
	...
	float yvel = 0;
	if (sonic->flags & EN_CEILING_FLAG) {
		yvel = 0;
		camera->ypos = sonic->ypos;
	} else if (!(sonic->flags & EN_VERTICAL_CONTACT_FLAG)) {
...
)
{
	...
	struct entity const * const tau_platform = &g->ents[EN_PLATFORM_TAU_ID];
	struct entity const * const omicron_platform = &g->ents[EN_PLATFORM_OMICRON_ID];
	struct entity const * const zeta_platform = &g->ents[EN_PLATFORM_ZETA_ID];
	struct entity * const block = &g->ents[id_block];
	...
	} else if (EN_BLOCK_IOTA_ID == block->id) {
		block->xpos = iota_platform->xpos;
		block->ypos = (
				iota_platform->ypos -
				(0.5f * iota_platform->height) -
	...
	} else if (EN_BLOCK_TAU_ID == block->id) {
		block->xpos = tau_platform->xpos;
		block->ypos = (
				tau_platform->ypos -
				(0.5f * tau_platform->height) -
	...
	} else if (EN_BLOCK_OMICRON_ID == block->id) {
		block->xpos = omicron_platform->xpos;
		block->ypos = (
				omicron_platform->ypos -
				(0.5f * omicron_platform->height) -
